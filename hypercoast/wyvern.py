...
def read_wyvern(filepath, bands=None, stac_url=None, **kwargs):
    """
    Read Wyvern COG GeoTIFF hyperspectral data and return an xarray.Dataset.

    Parameters:
        filepath (str or Path): Local file path or HTTPS URL to the geotiff file.
    ...
    if stac_url is None:
        ...
        stac_url = "https://wyvern-prod-public-open-data-program.s3.ca-central-1.amazonaws.com/wyvern_dragonette-003_20250508T203730_3cb1306b/wyvern_dragonette-003_20250508T203730_3cb1306b.json"

    # Parse STAC metadata
    stac_item = requests.get(stac_url, timeout=10).json()

    bands_meta = stac_item["assets"]["Cloud optimized GeoTiff"]["eo:bands"]
    ...
    # Apply affine transform to get x/y in projection coords
    x, y = transform * (cols, rows)

    # Reproject x/y to lat/lon if needed
    ...
    if bands is not None:
        data_float = data[bands]
        data = data_float.astype("int32")  # Convert to int to speed up rendering
...
def grid_wyvern(
    ...
    **kwargs,
    ...
    """
    Grids a Wyvern dataset based on latitude and longitude.

    Args:
        dataset (xr.Dataset): The Wyvern dataset to grid.
    ...
    Returns:
        xr.DataArray: The gridded Wyvern data.
    """
    ...
    if not np.any(overall_valid_mask):
        ...
        gridded_data_dict = {}
        scale = 4  # downsample factor (4x = 16x fewer pixels)

        for wl in selected_wavelengths:
            print(f"Band {wl} has been spatially downsampled by a factor of {scale}.")
            data = dataset.sel(wavelength=wl, method="nearest")["toa_radiance"]

            # Downsample spatial data to speed up interpolation
            data_ds = data.coarsen(x=scale, y=scale, boundary="trim").mean()
            lat_ds = dataset.latitude.coarsen(x=scale, y=scale, boundary="trim").mean()
            lon_ds = dataset.longitude.coarsen(x=scale, y=scale, boundary="trim").mean()

            # Mask nodata
            data_flat = data_ds.data.flatten()
            ...
            if not np.any(valid_mask):
                ...
                gridded_data = griddata(
                    (
                        lat_ds.data.flatten()[valid_mask],
                        lon_ds.data.flatten()[valid_mask],
                    ),
...
def wyvern_to_image(
    ...
    **kwargs,
    ...
    Args:
        dataset (xarray.Dataset or str): The dataset containing the Wyvern data or the file path to the dataset.
    ...
    if isinstance(dataset, str):
        dataset = read_wyvern(dataset, bands=bands)
...
def filter_wyvern(dataset, latitude, longitude, drop=True, return_plot=False, **kwargs):
    """
...
def extract_wyvern(
    ...
    **kwargs,
    ...
    """
    Extracts data from a Wyvern dataset for a given latitude and longitude range
        and calculates the mean over these dimensions.

    Args:
        dataset (Union[xr.Dataset, str]): The Wyvern dataset or path to the dataset file.
    ...
    ds = filter_wyvern(dataset, latitude, longitude, return_plot=False)
